...
class NGReconstructor():
    ...
    # def reconstruct(self, input_gradient, all_label, use_pbar=True):
    #     # one by one
    #     z_ress, x_ress, img_ress, loss_ress = [], [], [], []
    ...
    #         z_ress.append(z_res)
    #         x_ress.append(x_res)
    #         img_ress.append(img_res)
    #         loss_ress.append(loss_res)
    #
    #     return z_ress, x_ress, img_ress, loss_ress
    ...
    def reconstruct(self, input_gradient, labels, use_pbar=True):
        ...
        img_ress = []
        # print(z_res.shape)
        for i in range(z_res.shape[1]):
            with torch.no_grad():
                # x_res = self.generator(z_res.float(), c.float(), 1)
                # print(z_res[:,i].unsqueeze(0).shape)
                # print(c[i].unsqueeze(0).shape)
                x_res = self.generator(z_res[:,i].unsqueeze(0).float(), c[i].unsqueeze(0).float(), 1)
            x_res = nn.functional.interpolate(x_res, size=(224, 224), mode='area')
            img_res = convert_to_images(x_res.cpu())
            x_ress.append(x_res)
            img_ress.append(img_res)

        return z_res, x_ress, img_ress, loss_res
    ...
    def ng_loss(z, # latent variable to be optimized
        ...
        for i in range(z.shape[1]):
            ...
            # print(c[i].shape)
            with torch.no_grad():
                x = generator(z[:,i].unsqueeze(0), c[i].unsqueeze(0).float(), 1)

            x = nn.functional.interpolate(x, size=(224, 224), mode='area')
            xx.append(x)
        xx = torch.stack(xx).squeeze(1)
        # print(xx.shape)

        # compute the trial gradient
        target_loss, _, _ = loss_fn(fl_model(xx), labels)
        trial_gradient = torch.autograd.grad(target_loss, fl_model.parameters())
        trial_gradient = [grad.detach() for grad in trial_gradient]
